...
param prefix_flag - indicate if the prefix mode is active
param append_flag - indicate if the append mode is active
returns- the length of the file that has been printed
*/
int handleFile(int output_fd,linux_dirent* file,int debug_flag, int prefix_flag, int append_flag,char type){
  int return_val1=1;
  int return_val2=1;
  ...
  else if(append_flag){infector(file->name);}
...
{
  int output= STDOUT;
  int fd,debug_flag,prefix_flag,i,valid_argument,dirLength,append_flag;
  ...
  if( strncmp("-a",argv[i],2*sizeof(char))==0){
    valid_argument=1;
    append_flag=1;
    prefix=argv[i]+2*sizeof(char);
  }
  ...
  while(i<dirLength){
    ...
    if(strcmp(file->name,".")!=0 && strcmp(file->name,"..")!=0 ){ /* discarding prints of the root and the prev directory file descriptor*/
      if( (prefix_flag || append_flag)){
        if(strncmp(file->name,prefix, strlen(prefix))==0){
          handleFile(output,file,debug_flag,prefix_flag,append_flag,type);
        }
      }
      else if(!prefix_flag){
        handleFile(output,file,debug_flag,prefix_flag,0,type);
      }
    }
    i=i+file->len;
  }
