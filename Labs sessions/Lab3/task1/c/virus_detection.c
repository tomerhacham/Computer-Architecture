...
link* print_scenario(link* list, FILE* dummy_file){
...
link* load_scenario(link* link,FILE* dummy_file){
    char* filename=NULL;
    FILE* file;
...
link* quit(link* list,FILE* dummy_file){
    list_free(list);
    exit(EXIT_SUCCESS);
    return NULL;
}
...
link* detect_virus_scenario(link* sig_list, FILE* file){
...
int main(int argc, char const *argv[]) {
    ...
    MenuItem menu[] = { { "Load signatures", load_scenario },{ "Print signature", print_scenario },
                        {"Detect viruses",detect_virus_scenario },{ "Quit", quit }, { NULL, NULL } };
    ...
    while(1){
        ...
        if(picked_option==-1){//handle invalid argument
            ...
            list = menu[picked_option].fun(list,suspected_file);
        }
    }
    return 0;
}
