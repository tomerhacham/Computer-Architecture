...
void printRelocationTables(){
    ...
    if(rel_dyn_entry!=NULL){
        printf(" Offset\t\t  Info\n");
        for(int i=0; i<rel_dyn_entry->sh_size/sizeof(Elf32_Rel);i++){
            Elf32_Rel* entry = map_start+rel_dyn_entry->sh_offset+(i*sizeof(Elf32_Rel));
            printf("%08x\t%08x\n",entry->r_offset, entry->r_info);
        }
    }
    if(rel_plt_entry!=NULL){
        printf(" Offset\t\t  Info\n");
        for(int i=0; i<rel_plt_entry->sh_size/sizeof(Elf32_Rel);i++){
            Elf32_Rel* entry = map_start+rel_plt_entry->sh_offset+(i*sizeof(Elf32_Rel));
            printf("%08x\t%08x\n",entry->r_offset, entry->r_info);
        }
    }
}
...
int main(int argc, char **argv){
  fun_desc menu[] = { { "Toggle Debug Mode", toggleDebugMode }, {"Examine ELF File",examineFile},{"Print Section Names",printSectionTable},
                      {"Print Symbols",printSymbolTable},{"Relocation Tables",printRelocationTables},{ "Quit", quit } ,{ NULL, NULL } };
  size_t index=0;
